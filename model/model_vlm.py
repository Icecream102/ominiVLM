...
from .torch_compat import ensure_torch_transformers_compat
...
class VLMConfig(MiniMindConfig):
    ...
    def __init__(self, image_special_token='<|image_pad|>', image_ids=[12], **kwargs):
        ...
        self.projector_type = kwargs.get("projector_type", "mlp")
        super().__init__(**kwargs)

class MMVisionProjector(nn.Module):
    def __init__(self, in_dim, out_dim, source_tokens=64, target_tokens=64, projector_type="mlp"):
        super().__init__()
...
class MiniMindVLM(MiniMindForCausalLM):
    ...
    def image2tensor(image, processor):
        ...
        try:
            ...
            # A few managed GPU images ship a Torch/NumPy ABI combination for
            # which torch.from_numpy(np.ndarray) fails. Keep the fast path for
            # normal environments and use a dependency-free conversion only
            # for that specific processor failure.
            if "np.ndarray" not in str(error) and "Unable to create tensor" not in str(error):
                raise
            raw_inputs = processor(images=image, return_tensors=None)
            converted = {}
    ...
    def get_image_embeddings(image_inputs, vision_model):
        ...
        train_vision = torch.is_grad_enabled() and any(
            parameter.requires_grad for parameter in vision_model.parameters()
        )
        with (nullcontext() if train_vision else torch.no_grad()):
