...
本版本在基础 MiniMind-V 上补齐了可复现的 **Pretrain → SFT → GRPO → COCO500** 闭环：

- JSON 实验配置与一键可恢复流水线，训练权重和 optimizer 断点采用原子写入；
- 原生 GRPO 后训练，包含 group-relative advantage、clipped objective、冻结参考策略、自适应 KL 系数和 KL safety stop；
- COCO Caption 质量、推理吞吐、峰值显存以及 correct/black/shuffled 视觉依赖消融；
- 完整实验日志、逐样本预测、负实验诊断、模型卡、简历材料、单元测试与 GitHub Actions CI。

```bash
# 检查配置和命令，不启动训练
python scripts/run_experiment.py --config configs/full_pipeline_5090.json --dry_run

# 严格按阶段执行；中断后再次执行会从 done marker / resume checkpoint 恢复
python scripts/run_experiment.py --config configs/full_pipeline_5090.json
...
核心设计见 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)，模型适用范围见 [`MODEL_CARD.md`](MODEL_CARD.md)，正式结果见 [`benchmark_results/full_pipeline_20260811/REPORT.md`](benchmark_results/full_pipeline_20260811/REPORT.md)，简历表述与面试展开点见 [`docs/RESUME.md`](docs/RESUME.md)。
...
`eval_vlm.py` 是生成效果的人工检查工具，仅输出回答和推理速度，并不计算质量分数。
如需在**独立的、带参考答案的 Parquet 测试集**上量化评估 SFT 权重，可运行：

```bash
python eval_sft_vlm_metrics.py \
  --data_path dataset/sft_i2t_test.parquet \
  --load_from model --weight sft_vlm \
  --results_file eval_results_sft_vlm.jsonl \
  --metrics_file eval_metrics_sft_vlm.json
```

该脚本按中文 `jieba` 分词，计算 BLEU-1 至 BLEU-4、METEOR（精确词匹配版本）、ROUGE-L 和 CIDEr；逐样本预测写入 JSONL，汇总指标写入 JSON。`data_path` 必须是未参与训练的测试集，字段格式与 `dataset/sft_i2t.parquet` 一致；对训练集评估只会得到偏高的拟合分数。

使用 AIC-ICC 时，下载验证集图片与带 caption 的验证集标注 JSON 后，先转换（同张图的多条中文描述会全部保留为参考答案）：

```bash
python scripts/convert_aic_icc.py \
  --annotations /path/to/aic_icc_validation_annotations.json \
  --image_dir /path/to/aic_icc_validation_images \
  --output dataset/aic_icc_val.parquet

python eval_sft_vlm_metrics.py \
  --data_path dataset/aic_icc_val.parquet \
  --load_from model --weight sft_vlm
```

### COCO2017 统一基准与视觉依赖消融

`eval_vlm_benchmark.py` 支持 COCO Caption 标注，并在同一批样本上测试三种输入：正确图像、全黑图像、确定性错配图像。后两组用于判断模型是否真正使用视觉信息，而非只依赖语言先验：

```bash
python eval_vlm_benchmark.py \
  ...
  --conditions correct black shuffled \
  --output_dir benchmark_results
```

在单张 RTX 5090 上完成正式的 2-epoch Pretrain、2-epoch SFT、5000-step GRPO 后，以固定随机种子 42 抽取 COCO2017 validation 的 500 张图，使用贪心解码和同一英文 Caption prompt，实测结果如下：

| 权重 | BLEU-1 | BLEU-4 | METEOR-exact | ROUGE-L | CIDEr-style | tokens/s | 峰值显存 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Pretrain | **0.2786** | **0.0366** | 0.2978 | **0.2563** | **0.01390** | **206.5** | 358.2 MB |
| SFT | 0.2767 | 0.0346 | **0.2995** | 0.2555 | 0.01179 | 198.1 | 358.2 MB |
| GRPO | 0.1694 | 0.0104 | 0.2012 | 0.1638 | 0.00028 | 178.5 | 357.4 MB |

| SFT 视觉条件 | BLEU-4 | ROUGE-L | 输出变化率 | 与正确图输出的 token Jaccard 距离 |
|---|---:|---:|---:|---:|
| 正确图 | 0.0346 | 0.2555 | - | - |
| 全黑图 | 0.0000 | 0.1647 | 100% | 0.8720 |
| 错配图 | 0.0069 | 0.2086 | 100% | 0.8020 |

正式实验中，SFT 仅在 METEOR-exact 上略高于 Pretrain，其余 Caption 指标小幅下降；GRPO 的末 500 步代理奖励较前 500 步提高约 23.5%，但 COCO 指标明显退化，且 KL 从前 500 步均值 0.0761 增至末 500 步的 0.8380。这说明当前 GRPO 奖励与通用 Caption 质量存在错位，`beta=0.02` 的约束也不足，不能把代理奖励上升表述为模型能力提升。正确图被替换后所有模型的输出均发生变化，说明输出依赖视觉输入。完整配置、逐样本预测、原始日志和改进建议见 [`benchmark_results/full_pipeline_20260811/REPORT.md`](benchmark_results/full_pipeline_20260811/REPORT.md)。这些指标是项目内自包含实现，适合模型间受控对比；与论文数字横向比较时应改用官方 COCOEvalCap。

### 官方 COCOEvalCap 基准（2026-08-12）

在 COCO2017 val2017 全量 5000 张图上，用官方 COCOEvalCap（pycocoevalcap）对 6 个检查点完成评测（贪心解码，max_new_tokens=48，英文 Caption prompt）：

| 权重 | BLEU-1 | BLEU-4 | ROUGE-L | CIDEr |
|---|---:|---:|---:|---:|
| Pretrain | 0.2311 | **0.0250** | **0.2160** | **0.0063** |
| SFT | **0.2367** | 0.0237 | 0.2136 | 0.0058 |
| GRPO（beta=0.02） | 0.1561 | 0.0067 | 0.1517 | 0.0002 |
| GRPO 修复（group=8 + 自适应 KL） | 0.2262 | 0.0201 | 0.2016 | 0.0016 |
| **GRPO fix2（group=16 + CIDEr 奖励）** | **0.2392** | **0.0266** | **0.2159** | 0.0036 |

GRPO 修复实验将奖励错位导致的退化显著缓解：官方 BLEU-4 从原版 0.0067 恢复到 **0.0266（fix2，超过 SFT 的 0.0237）**，KL 由末步 1.23 压至 0.04。完整对照（含 beta=0.10、自适应 KL、group=16 + CIDEr 奖励变体）、VQAv2 评测、架构消融与可复现命令见 [`benchmark_results/official_coco_20260812/REPORT.md`](benchmark_results/official_coco_20260812/REPORT.md)。

VQAv2 多任务对齐：基线 0 → VQA 专项 SFT **31.6%**（但 caption 灾难性遗忘）→ VQA+caption 混合 SFT **20.9%** 且官方 COCO CIDEr **0.0067（全场最优）**、BLEU-4 0.0241（双任务同时保留）。

规模对比：**Qwen2.5-VL-3B + LoRA**（0.98% 可训练参数、28 分钟/单卡）VQAv2 **82.0%**、官方 COCO CIDEr **0.8364**（较 65M 提升 120×）——完整对比与数据画像见 [`benchmark_results/official_coco_20260812/REPORT.md`](benchmark_results/official_coco_20260812/REPORT.md)。

MMBench en/dev 综合基准：3B LoRA **84.9%**（4329 题全量）vs 65M 12.0%（200 题抽样）。
