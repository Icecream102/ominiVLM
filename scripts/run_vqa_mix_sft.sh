...
SEED="${SEED:-42}"
NAME="vqa_sft_mix_vlm"

mkdir -p logs/vqa checkpoints/vqa results/vqa results/official_coco out

if [[ ! -f dataset/vqa_caption_mix.parquet ]]; then
  "$PYTHON_BIN" scripts/build_mixed_sft_dataset.py \
    --vqa_parquet dataset/vqa_sft.parquet \
    --base_parquet dataset/sft_i2t.parquet \
    --base_samples 20000 --seed "$SEED" \
    --output dataset/vqa_caption_mix.parquet
fi
...
if [[ ! -f "logs/vqa/${NAME}.done" ]]; then
  "$PYTHON_BIN" -u trainer/train_sft_vlm.py \
    --data_path dataset/vqa_caption_mix.parquet --save_dir out --checkpoint_dir checkpoints/vqa \
...
echo "VQA mixed SFT pipeline complete."
