...
mkdir -p logs/fix checkpoints/fix results/grpo_fix/coco500 out
...
if [[ ! -f "logs/fix/${NAME}.done" ]]; then
  "$PYTHON_BIN" -u trainer/train_grpo_vlm.py \
    ...
    --epochs 1 --group_size 8 --ppo_epochs 2 --learning_rate 1e-6 \
    --clip_eps 0.2 --beta 0.02 --adaptive_beta 1 --target_kl 0.08 \
    --beta_update_rate 0.02 --min_beta 0.01 --max_beta 0.50 \
    --kl_stop 0.60 --kl_stop_patience 50 \
    --reward_unigram 0.15 --reward_rouge 0.35 --reward_meteor 0.35 \
    --reward_length 0.05 --reward_repetition 0.10 \
...
"$PYTHON_BIN" -u eval_vlm_benchmark.py \
  ...
  --output_dir results/grpo_fix/coco500 \
  2>&1 | tee "logs/fix/eval_${NAME}.log"

echo "GRPO fix run complete."
