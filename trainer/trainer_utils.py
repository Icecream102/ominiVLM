...
def init_vlm_model(vlm_config, from_weight='pretrain_vlm', tokenizer_path='../model', vision_model_path='../model/siglip2-base-p32-256-ve', save_dir='../out', device='cuda', freeze_llm=0, vision_unfreeze_layers=0):
    ...
    if vision_unfreeze_layers:
        encoder = model.vision_encoder.vision_model.encoder
        layers = encoder.layers
        if vision_unfreeze_layers > len(layers):
            raise ValueError(
                f"vision_unfreeze_layers={vision_unfreeze_layers} exceeds {len(layers)}"
            )
        for layer in layers[-vision_unfreeze_layers:]:
...
def inference_state_dict(model):
    """Keep all non-vision weights and only the vision parameters that were trained."""
    trainable = {name for name, parameter in model.named_parameters() if parameter.requires_grad}
    return {
        key: value for key, value in model.state_dict().items()
        if not key.startswith('vision_encoder.') or key in trainable
    }


def build_vlm_optimizer(model, learning_rate, vision_learning_rate=None):
    vision, other = [], []
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        (vision if name.startswith("vision_encoder.") else other).append(parameter)
    groups = [{"params": other, "lr_scale": 1.0}]
    if vision:
        vision_lr = vision_learning_rate or learning_rate
        groups.append({
            "params": vision,
            "lr_scale": vision_lr / learning_rate,
        })
