...
from trainer.trainer_utils import get_lr, Logger, is_main_process, init_distributed_mode, setup_seed, init_vlm_model, vlm_checkpoint, SkipBatchSampler, vlm_collate_fn, inference_state_dict, build_vlm_optimizer
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, pixel_values) in enumerate(loader, start=start_step + 1):
        ...
        pixel_values = {k: v.to(args.device, non_blocking=True) for k, v in pixel_values.items()} if isinstance(pixel_values, dict) else pixel_values.to(args.device, non_blocking=True)
        last_step = step
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            clean_state_dict = inference_state_dict(raw_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints", help="断点保存目录")
    parser.add_argument("--tokenizer_path", type=str, default="model", help="tokenizer目录")
    parser.add_argument("--vision_model_path", type=str, default="model/siglip2-base-p32-256-ve", help="视觉编码器目录")
    ...
    parser.add_argument("--vision_learning_rate", type=float, default=1e-5)
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu", help="训练设备")
    parser.add_argument("--dtype", type=str, default="bfloat16", help="混合精度类型")
    parser.add_argument("--num_workers", type=int, default=8, help="数据加载线程数")
    parser.add_argument("--prefetch_factor", type=int, default=8, help="每个worker预取batch数")
    parser.add_argument("--persistent_workers", default=1, type=int, choices=[0, 1], help="是否复用DataLoader workers")
    parser.add_argument("--accumulation_steps", type=int, default=1, help="梯度累积步数")
    parser.add_argument("--grad_clip", type=float, default=1.0, help="梯度裁剪阈值")
    parser.add_argument("--log_interval", type=int, default=100, help="日志打印间隔")
    parser.add_argument("--save_interval", type=int, default=20000, help="模型保存间隔")
    parser.add_argument("--max_steps", type=int, default=0, help="每个epoch最大训练步数，0表示完整数据")
    ...
    parser.add_argument('--projector_type', default='mlp', choices=['linear', 'mlp'])
    parser.add_argument('--vision_unfreeze_layers', default=0, type=int)
    ...
    vlm_config = VLMConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers, max_seq_len=args.max_seq_len, use_moe=bool(args.use_moe), projector_type=args.projector_type)
    ...
    optimizer = build_vlm_optimizer(model, args.learning_rate, args.vision_learning_rate)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        effective_iters = min(len(loader) + skip, args.max_steps) if args.max_steps else len(loader) + skip
        if skip > 0: 
            Logger(f'Epoch [{epoch + 1}/{args.epochs}]: 跳过前{start_step}个step，从step {start_step + 1}开始')
            train_epoch(epoch, loader, effective_iters, start_step, wandb)
        else:
            train_epoch(epoch, loader, effective_iters, 0, wandb)
    
    # ========== 9. 清理分布进程 ==========
    if is_main_process():
        peak_vram = torch.cuda.max_memory_allocated() / 1024 ** 3 if torch.cuda.is_available() else 0.0
        Logger(f'Training complete, elapsed: {(time.time() - training_started_at) / 3600:.3f}h, peak_vram: {peak_vram:.3f}GB')
