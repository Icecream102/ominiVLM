...
For reproducible quantitative evaluation, `eval_vlm_benchmark.py` evaluates the same COCO Caption examples under correct-image, black-image, and deterministically shuffled-image conditions:
...
On one RTX 5090, 500 fixed COCO2017 validation examples (seed 42, greedy decoding) produced:

| Checkpoint | BLEU-1 | BLEU-4 | METEOR-exact | ROUGE-L | CIDEr-style | tokens/s | Peak VRAM |
|---|---:|---:|---:|---:|---:|---:|---:|
| Pretrain | 0.2827 | 0.0428 | 0.3039 | 0.2617 | 0.0158 | 186.6 | 357.4 MB |
| SFT | **0.2958** | **0.0435** | **0.3335** | **0.2715** | 0.0141 | **191.4** | 358.2 MB |

For SFT, BLEU-4 fell from 0.0435 to 0.0000 with black images and 0.0062 with shuffled images; all generated outputs changed. This controlled ablation provides evidence that the model uses visual input. The self-contained metrics are intended for controlled comparisons inside this project; use official COCOEvalCap for comparison with published papers. See [`benchmark_results/README.md`](benchmark_results/README.md) for configuration, ablations, and raw summaries.
